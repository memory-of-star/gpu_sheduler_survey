...
证据等级：**机制可行性证据；不是生产负载端到端收益结论**

> **2026-08-05 历史术语校准：**本报告保留了当时的“一波”和“理论空间”用语，以便审计原结论。按当前准入规则，`P,C ≤ SM` 只是 grid ratio，不保证一个实际调度 wave；Ceiling 只是 unsafe measured no-wait operational reference，不是数学/架构时间下界。本报告的小 grid 结果只作历史机制背景，完整多波 Gate 以 2026-08-05 v2 正式报告为准。
