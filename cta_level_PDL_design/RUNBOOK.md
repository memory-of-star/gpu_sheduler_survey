...
./run_all.sh tier0                           # ~1h
./run_all.sh tier1                           # ~2h   ← 决策点在这里
python3 ../tools/analyze.py results/summary.txt | tee /tmp/gate.txt

# ── 决策：Tier 1 的收益空间够不够 ─────────────────────────
# 够  → ./run_all.sh tier23 ; cd llm && ./run_llm_sweep.sh ; cd ../dsa && ./run_dsa_chain.sh
...
```bash
cd ../../跨stream_PDL调研/bench/pdl_bench && ./build.sh && ./run.sh | tee ~/b300_xstream.log
```
...
```
Degree boundary (last degree with >=2% space -> first below):
  grid=  1024: benefit persists to degree XXX, gone by YYY
```

BlockMaestro 在 28 SM 的 Titan X 上得出的是 32。B300 约 148 SM。**这个数往哪边移是决定性的**：
...
依赖度相同、只有形状不同。两者差距大 → 说明瓶颈是**编码**（A3 维度）而非依赖度。这是 BlockMaestro 的 n-组全连接注入无法做出的区分。

**(c) grid 规模边界**

BlockMaestro 是 2048 TB。同样看 B300 上往哪边移。
...
| **Tier 1 收益地图** | **2.0 h** | **否（决策点）** |
| Tier 2/3 机制对比 | 3.0 h | 视决策 |
